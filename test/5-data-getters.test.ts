...
describe('5. Data getters', async () => {
  ...
  const GETTER__USER_FULL_NAME = 'getUserFullName';
  const dataUserVO = {
    firstName: 'Wires',
    lastName: 'Ware',
  };
  ...
  Wire.data(GETTER__USER_FULL_NAME, null, wireDataGetter);

  test('5.1 Getter', async () => {
    print('>\t Wire.data(GET__USER_FULL_NAME).isLocked');
    expect(() => Wire.data(GETTER__USER_FULL_NAME).subscribe(async () => Promise.resolve())).toThrowError();
    expect(Wire.data(GETTER__USER_FULL_NAME).isLocked).toBeTruthy();
    expect(Wire.data(GETTER__USER_FULL_NAME).isGetter).toBeTruthy();
    ...
    expect(Wire.data(GETTER__USER_FULL_NAME).value).toBe(`${dataUserVO['firstName']} Cores`);
    expect(() => Wire.data(GETTER__USER_FULL_NAME, 'new value')).toThrowError();
  });
});
